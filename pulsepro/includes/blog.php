...
if(!empty($_POST['name']) && !empty($_POST['mail']) && !empty($_POST['comment']) &&  $blog_comments && ($too_long != true) ) {
	...
	if(preg_match("/@/", $_POST['mail'])) {
		...
		if($resp == 1) {
			...
			$s=0;
			$line = explode("\n", $out);
			$amount_of_lines = count($line);
			
			for($s=0; $s < $amount_of_lines; $s++) { 
				$item = explode("|", $line[$s]);
       			
        	if( ($item[0] == $id_post) && ($item[2] == $name) && ($item[3] == $email) && ($item[4] == $comment) ){ 
			...
			for($i=0; $i < $nb; $i++) { 
				$blog = explode("|", $line[$i]);
					if($blog[0] == $id_post) {$date = explode( ' ' , $blog[2]);
				...
				$mail_body = $lang_blog_notify . $title;
...
?>
	<form class="comment-form" action="<?php if($rewrite) {echo "blog-$id_post-$clean_url";} else {echo "?d=$id_post";}?>" method="post">
		<?php
			 ...
			 if ($too_long == true){echo "<span style=\"color:red\">Your comment is too long</span>";}
			
			 else if($error == 1 && isset($_POST['name']) && isset($_POST['mail'])) echo "<span style=\"color:red\">$lang_blog_error_check</span><br />";	
		...
		<p><label class="comment-label"><?php echo $lang_blog_label_name; ?></label><br />
		<input type="text" name="name" size="45" value="<?php echo strip_tags(trim(stripslashes($_POST['name']))); ?>" /></p>
		
		<p><label class="comment-label"><?php echo $lang_blog_label_email; ?></label><br />
		...
		<p><label class="comment-label"><?php echo $question; ?> </label> 
		...
		if (isset($blog[0]) & $blog[0] != '') {
			...
			if( $blog_comments) {
				...
				if($blog[1]) {
				    ...
				    if($rewrite) {
			            echo  "<div class=\"num_comments\"><a href=\"blog-". $blog[0] ."-$clean_url#comments\">(". $blog[1] .") $lang_blog_num_comment</a></div>\n\n";
			        } else {
		                echo  "<div class=\"num_comments\"><a href=\"?d=". $blog[0] ."#comments\">(". $blog[1] .") $lang_blog_num_comment</a></div>\n\n";
			        }	
					
				} else {
					if($rewrite) {
			            echo "<div class=\"num_comments\"><a href=\"blog-". $blog[0] ."-$clean_url#comments\">$lang_blog_no_comment</a></div>\n\n";
			        } else {
		                echo "<div class=\"num_comments\"><a href=\"?d=". $blog[0] ."#comments\">$lang_blog_no_comment</a></div>\n\n";
			        }	
